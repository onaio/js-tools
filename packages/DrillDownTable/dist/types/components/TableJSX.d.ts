...
export declare function hasChildrenFunc<D extends object>(cellObject: Cell<D>, parentIdList: Array<number | string>, idField?: string | number): boolean;
/** the actual table instance returned from UseTable() after including usePagination and useSortBy hooks */
export interface DrillDownInstanceProps<T extends object> extends Omit<UseTableInstanceProps<T>, 'state'>, UsePaginationInstanceProps<T>, UseSortByInstanceProps<T> {
...
export declare type FilterBarRenderer<TData extends object> = (prop: RenderFiltersInBarOptions<TData>) => ReactNode;
export declare type DrillDownColumn<T extends object> = Column<T> & UseSortByColumnOptions<T> & UseResizeColumnsColumnOptions<T> & UseSortByColumnOptions<T>;
...
export interface TableJSXProps<TData extends object> {
    ...
    fetchData: (options: Dictionary) => void /** function that dynamically changes the chunk of data given to react-table */;
...
export declare const defaultTableProps: Omit<TableJSXProps<{}>, 'columns' | 'fetchData' | 'parentNodes'>;
